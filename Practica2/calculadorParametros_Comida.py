...
# Ahora, cada diccionario de imagen contiene las 4 clases.
datos_por_imagen = [
    { # --- DATOS DE ENTRENAMIENTO1.JPG ---
      'total_pixeles_img': 360000, # <-- ¡IMPORTANTE!
      'chile': {
...
# Acumuladores para cada clase
acumuladores = {
    'chile':   {'N': 0, 'suma_medias_ponderadas': 0, 'suma_productos_externos': 0},
    'platano': {'N': 0, 'suma_medias_ponderadas': 0, 'suma_productos_externos': 0},
    'huevo':   {'N': 0, 'suma_medias_ponderadas': 0, 'suma_productos_externos': 0},
...
for datos_img in datos_por_imagen:
    ...
    for nombre_clase, params in datos_img.items():
        ...
        # Extraer datos y calcular N (número de píxeles)
        media, cov, prior = params['media'], params['cov'], params['prior']
        N = int(round(prior * datos_img['total_pixeles_img']))
        if N == 0: continue

        # Calcular la suma de productos externos (sum of outer products)
...
for nombre_clase, acum in acumuladores.items():
    ...
    parametros_finales[nombre_clase] = {
        'media': media_final, 'cov': cov_final, 'prior': prior_final
    }
...
for nombre_clase, params in parametros_finales.items():
    ...
    print(f"        'color': (0, 0, 0) # Define el color BGR para '{nombre_clase}'")
    print("    },")
print("}")