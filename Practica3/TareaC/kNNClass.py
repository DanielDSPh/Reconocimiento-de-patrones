...
class kNN():
    ...
    def getClassProbs(self, X_test):
        """
        Calcula las probabilidades a posteriori de clase para cada instancia en X_test.
        Devuelve un DataFrame con una columna por clase y una fila por instancia de X_test.
        """
        rows = []
        ...
        for i in range(len(X_test)):
            ...
            df_knn = df_dists.sort_values(by="dist").iloc[:self.k]
            ...
            row = [counts.get(c, 0.0) for c in self.classes_]
            rows.append(row)

        # DataFrame de probabilidades; index alineado a X_test
        probs_df = pd.DataFrame(rows, columns=self.classes_, index=X_test.index)
        return probs_df
